...
{
  /// <summary>
  /// Сервис управления переключением между стадиями эволюции симбионта
  ...
  public sealed class EvolutionStageService : IDisposable
  {
    ...
    /// <summary>
    /// Переключает симбионта на указанную стадию эволюции с очисткой данных последующих стадий
    /// </summary>
    ...
    public EvolutionStageChangeResult ChangeEvolutionStage(int targetStage, bool force = false, bool skipDataClearing = false)
    {
      ...
      try
      {
        ...
        if (targetStage == currentStage)
        {
          ...
          {
            return EvolutionStageChangeResult.CreateFailure(
                "Симбионт уже находится на указанной стадии");
          }
          else
    ...
    public void ClearStageDataOnlyForScenarioPreRun(int stage)
    {
      ...
      try
      {
        // Если симбионт временно на стадии 0–1, данные стадии 2 (автоматизмы) всё равно могли остаться в памяти/файлах
        // после предыдущего прогона на стадии ≥2. Ограничивать верхнюю границу только текущей стадией — значит
        // не вызывать ClearStageData(2) и не очищать автоматизмы (см. DeleteAllAutomatizm при EvolutionStage < 2).
