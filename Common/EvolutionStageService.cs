...
namespace ISIDA.Common
{
  ...
  public sealed class EvolutionStageService : IDisposable
  {
    ...
    /// <summary>
    /// Полная очистка накопленных данных агента перед запуском сценария
    /// (как при переходе на стадию 0 с max(текущая стадия, 2)): рефлексы, автоматизмы стадии 2 и т.д.
    /// Нужно, чтобы при агенте на стадии 0–1 всё равно сбрасывались автоматизмы после предыдущего прогона на стадии ≥2.
    ...
    public void ClearStageDataOnlyForScenarioPreRun(int stage)
    {
      ...
      try
      {
        // Если агент временно на стадии 0–1, данные стадии 2 (автоматизмы) всё равно могли остаться в памяти/файлах
